...
template <typename Facade, typename Heap>
void insertSourceForwardNode(const Facade &facade,
                             Heap &heap,
...
template <typename Facade, typename Heap>
void insertSourceReverseNode(const Facade &facade,
                             Heap &heap,
                             const PhantomNode &node,
                             const EdgeWeight initial_weight)
...
template <typename Facade, typename Heap>
void insertTargetForwardNode(const Facade &facade,
                             Heap &heap,
                             const PhantomNode &node,
                             const EdgeWeight initial_weight)
...
template <typename Facade, typename Heap>
void insertTargetReverseNode(const Facade &facade,
                             Heap &heap,
                             const PhantomNode &node,
                             const EdgeWeight initial_weight)
{ // Insert target node in forward direction into a backward heap
                        ...
                        const PhantomNodes &nodes)
{
    ...
    {
        insertSourceForwardNode(facade, forward_heap, source, 0);
    }

    if (source.reverse_segment_id.enabled && !cross_reverse)
    {
        insertSourceReverseNode(facade, forward_heap, source, 0);
    }

    if (target.forward_segment_id.enabled && !cross_forward)
    {
        insertTargetForwardNode(facade, reverse_heap, target, 0);
    }

    if (target.reverse_segment_id.enabled && !cross_reverse)
    {
        insertTargetReverseNode(facade, reverse_heap, target, 0);
    }
...
template <typename Facade, typename Heap>
auto insertNodesInHeapsToTargetForward(const Facade &facade,
                                       Heap &forward_heap,
                                       Heap &reverse_heap,
                                       const PhantomNode &source,
                                       const PhantomNode &target,
                                       EdgeWeight weight_to_source_forward,
                                       EdgeWeight weight_to_source_reverse)
{
    const bool forward_valid = weight_to_source_forward != INVALID_EDGE_WEIGHT;
    const bool reverse_valid = weight_to_source_reverse != INVALID_EDGE_WEIGHT;
    ...
    {
        insertSourceForwardNode(facade, forward_heap, source, weight_to_source_forward);
    }

    if (source.reverse_segment_id.enabled && reverse_valid)
    {
        insertSourceReverseNode(facade, forward_heap, source, weight_to_source_reverse);
    }
...
template <typename Facade, typename Heap>
auto insertNodesInHeapsToTargetReverse(const Facade &facade,
                                       Heap &forward_heap,
                                       Heap &reverse_heap,
                                       const PhantomNode &source,
                                       const PhantomNode &target,
